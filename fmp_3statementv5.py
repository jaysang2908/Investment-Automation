...
GEMINI_KEY   = "YOUR_GEMINI_KEY"    # replace before running locally
...
def fetch_credit_rating_web(ticker):
    """Search DuckDuckGo HTML for S&P / Moody's issuer credit rating.
    Returns (sp_rating_or_None, source_string)."""
    import re, urllib.parse
    query = urllib.parse.quote(f"{ticker} S&P Moody's issuer credit rating")
    url   = f"https://html.duckduckgo.com/html/?q={query}"
    hdrs  = {"User-Agent": ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                            "AppleWebKit/537.36 (KHTML, like Gecko) "
                            "Chrome/120.0.0.0 Safari/537.36")}
    try:
        text = requests.get(url, headers=hdrs, timeout=10).text
        # 1. Moody's notation — very distinctive, low false-positive risk
        for m_r, sp_r in MOODY_TO_SP.items():
            if re.search(rf'\b{re.escape(m_r)}\b', text):
                return sp_r, f"Web search (Moody's {m_r})"
        # 2. S&P notation — context-anchored to reduce false positives
        for pat in [
            r"(?:S&P|Standard\s*&\s*Poor)[^\w].*?(AA[A+-]?|A[A+-]?|BBB[+-]?|BB[+-]?|B[+-]?|CCC[+-]?)",
            r"(?:rated?|rating)[:\s]+(AA[A+-]?|A[A+-]?|BBB[+-]?|BB[+-]?|B[+-]?|CCC[+-]?)",
            r"(AA[A+-]?|A[A+-]?|BBB[+-]?|BB[+-]?)\s+(?:rating|rated|issuer|outlook)",
        ]:
            m = re.search(pat, text, re.IGNORECASE)
            if m:
                cand = m.group(1).upper()
                if cand in VALID_SP_RATINGS:
                    return cand, f"Web search (S&P {cand})"
        return None, "Not found via web search"
    except Exception as e:
        return None, f"Web search failed: {str(e)[:50]}"
...
def get_ai_suggestion(prompt):
    """Call Gemini REST API — tries models in order until one succeeds."""
    if not GEMINI_KEY:
        return None
    import time
    models = [
        "gemini-2.5-flash",       # primary — fast, free tier
        "gemini-2.5-flash-lite",  # fallback — high volume, low cost
        "gemini-2.5-pro",         # fallback — higher reasoning, 50 req/day limit
    ]
    for model in models:
        try:
            ...
            # Any retryable / availability error — try next model
            print(f"  [AI] {model} {r.status_code} — trying next model...")
            time.sleep(1)
            continue
        except Exception as e:
            print(f"  [AI] {model} failed: {e}")
            continue
    print("  [AI] All models exhausted — AI suggestions unavailable.")
    return None
...
def build_wacc(wb, ticker, is_data, bs_data, manual_rating=None):
    ...
    # Credit rating — manual input takes priority, then FMP, then AI fallback
    if manual_rating:
    ...
    # ── AI suggestions ────────────────────────────────────────────────────────
    print("  Requesting AI suggestions from Gemini...")

    # Credit rating (if FMP didn't return one)
    if not fmp_rating:
        rating_prompt = (
            f"What is the current S&P or Moody's issuer credit rating for {ticker}? "
            f"Reply with ONLY the rating symbol (e.g. AA+ or Aa1) and nothing else."
        )
        rating_resp = get_ai_suggestion(rating_prompt)
        if rating_resp:
            tok = rating_resp.strip().split()[0].strip(".,;:()")
            normalized = MOODY_TO_SP.get(tok) or (tok.upper() if tok.upper() in VALID_SP_RATINGS else None)
            if normalized:
                fmp_rating   = normalized
                rating_source = f"Gemini AI ({tok})"
                print(f"    AI-sourced rating: {fmp_rating}")

    def parse_ai(resp):
        """Extract VALUE and RATIONALE lines from Gemini response."""
        if not resp:
            return None, "AI suggestion unavailable — check GEMINI_KEY."
        try:
            lines = resp.strip().split("\n")
            val_line = next(l for l in lines
                            if l.upper().startswith("VALUE:"))
            rat_line = next(l for l in lines
                            if l.upper().startswith("RATIONALE:"))
            return (float(val_line.split(":", 1)[1].strip()),
                    rat_line.split(":", 1)[1].strip())
        except Exception:
            return None, (resp[:220] if resp else
                          "AI suggestion unavailable.")

    # Beta
    beta_prompt = (
        f"You are a financial analyst building a DCF model for {ticker} "
        f"(sector: {sector}).\n"
        f"Beta data points:\n"
        f"- Raw 5yr beta (FMP): {raw_beta}\n"
        f"- Blume-adjusted (0.67*raw+0.33): {blume}\n"
        f"- Damodaran industry unlevered ({sector}): {dama_unlevered}\n"
        f"- Damodaran re-levered (D/E={de_ratio:.2f}x): {dama_relevered}\n"
        f"- Peer median ({', '.join(p for p,_ in peer_betas)}): {peer_median}\n"
        f"Suggest one forward-looking beta for CAPM. "
        f"Respond ONLY as:\nVALUE: X.XX\nRATIONALE: [2-3 sentences]"
    )
    beta_ai_val, beta_ai_rat = parse_ai(get_ai_suggestion(beta_prompt))

    # ERP
    erp_prompt = (
        f"You are a financial analyst setting ERP for a US DCF model ({ticker}).\n"
        f"- Damodaran implied ERP (current): {DAMODARAN_ERP_IMPLIED*100:.2f}%\n"
        f"- Damodaran historical avg 1928-2025: {DAMODARAN_ERP_HIST_AVG*100:.2f}%\n"
        f"- Current 10yr Treasury: {rf*100:.2f}%\n"
        f"Suggest one ERP. "
        f"Respond ONLY as:\nVALUE: 0.XXXX  (decimal, e.g. 0.0500)\n"
        f"RATIONALE: [2-3 sentences]"
    )
    erp_ai_val, erp_ai_rat = parse_ai(get_ai_suggestion(erp_prompt))

    # Rd
    net_int = int_inc - int_exp
    rd_prompt = (
        f"You are a financial analyst estimating pre-tax cost of debt for {ticker}.\n"
        f"- FMP credit rating: {fmp_rating or 'Not available'}\n"
        f"- FRED rating-matched yield: "
        f"{f'{fred_rd*100:.2f}%' if fred_rd else 'N/A'} ({fred_rd_src})\n"
        f"- Synthetic rating (ICR {icr:.1f}x): {synth_rating}, "
        f"Rd = {rd_synthetic*100:.2f}%\n"
        f"- Accounting Rd (gross IntExp/AvgDebt): "
        f"{f'{rd_acctg*100:.2f}%' if rd_acctg else 'N/A'}\n"
        f"- Net interest: {'INCOME' if net_int>0 else 'EXPENSE'} "
        f"of ${abs(net_int)/1e6:.0f}mm\n"
        f"- Debt weight in capital structure: {w_d*100:.1f}%\n"
        f"Suggest one pre-tax Rd for WACC. "
        f"Respond ONLY as:\nVALUE: 0.XXXX\nRATIONALE: [2-3 sentences]"
    )
    rd_ai_val, rd_ai_rat = parse_ai(get_ai_suggestion(rd_prompt))

    # Defaults if AI unavailable
    sel_beta = beta_ai_val or blume or raw_beta or 1.0
    sel_erp  = erp_ai_val  or DAMODARAN_ERP_IMPLIED
    sel_rd   = rd_ai_val   or fred_rd or rd_synthetic or rd_acctg or 0.05

    # ── Write sheet ───────────────────────────────────────────────────────────
    row = 1
    row = write_tab_title(
        ws, row, f"{ticker.upper()} — WACC & COST OF CAPITAL", NC,
        subtitle=("Blue = user override  |  Amber = AI recommendation  |  "
                  "Green = selected input  |  All figures USD millions"))
    row = blank(row)
    ...
    row = wrow(row, "AI recommendation  (Gemini 2.0 Flash)",
               beta_ai_val, "Rationale below",
               bg=C_AI_BG,
               val_color=C_BLACK if beta_ai_val else "999999")
    row = rat_row(row, f"Rationale: {beta_ai_rat}")
    ...
    row = input_row(row, "► Selected beta  (user override)",
                    round(sel_beta, 3),
                    "Pre-filled with AI suggestion — override freely")
    row = blank(row)
    ...
    row = wrow(row, "AI recommendation  (Gemini 2.0 Flash)",
               erp_ai_val, "Rationale below",
               bg=C_AI_BG, is_pct=True,
               val_color=C_BLACK if erp_ai_val else "999999")
    row = rat_row(row, f"Rationale: {erp_ai_rat}")
    erp_row = row
    ...
    row = wrow(row, "AI recommendation  (Gemini 2.0 Flash)",
               rd_ai_val, "Rationale below",
               bg=C_AI_BG, is_pct=True,
               val_color=C_BLACK if rd_ai_val else "999999")
    row = rat_row(row, f"Rationale: {rd_ai_rat}")
    rd_row = row
